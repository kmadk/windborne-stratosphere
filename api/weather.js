...
async function fetchOpenMeteoWindData() {
  ...
  return {
    ...
    metadata: {
      ...
      description:
        "Real-time atmospheric wind data at 250hPa pressure level (jet stream altitude)",
...
async function fetchWindAtPoint(lat, lon, type) {
  try {
    ...
    const windSpeed250 =
      data.hourly?.wind_speed_250hPa?.[currentHour] ||
      data.hourly?.wind_speed_250hPa?.[0];
    const windDir250 =
      data.hourly?.wind_direction_250hPa?.[currentHour] ||
    ...
    if (windSpeed250 === undefined || windDir250 === undefined) {
      ...
      const windSpeed500 =
        data.hourly?.wind_speed_500hPa?.[currentHour] ||
        data.hourly?.wind_speed_500hPa?.[0];
      const windDir500 =
        data.hourly?.wind_direction_500hPa?.[currentHour] ||
...
function generateFallbackData() {
  ...
  const bands = [
    {
      lat: 45,
      type: "polar_jet_north",
      altitude: 10000,
      speedRange: [80, 120],
    },
    {
      lat: 30,
      type: "subtropical_jet_north",
      altitude: 12000,
      speedRange: [60, 90],
    },
    {
      lat: -30,
      type: "subtropical_jet_south",
      altitude: 12000,
      speedRange: [50, 80],
    },
    {
      lat: -50,
      type: "polar_jet_south",
      altitude: 10000,
      speedRange: [90, 140],
    },
  ];
  ...
  bands.forEach((band) => {
    for (let lon = -180; lon <= 180; lon += 5) {
      ...
      jetStreams.push({
        ...
        windSpeed:
          band.speedRange[0] +
          Math.random() * (band.speedRange[1] - band.speedRange[0]),
